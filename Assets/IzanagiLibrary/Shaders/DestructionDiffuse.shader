﻿Shader "IzanagiShader/Lit/DestructionDiffuse"
{
	Properties
	{
		...
		_DiffuseFactor("Diffuse Factor", Range(0.0, 2.0)) = 1.0
...
#define SPREADINGFACTOR 0.5
	...
	void geom(triangle appdata_t input[3], inout TriangleStream<g2f> stream)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			o.color = nl * _LightColor0 * (destruction * 5.0 + 1.0);
	...
	fixed4 frag(g2f i) : SV_Target
	{
		...
		fixed4 col = _Color * i.color * tex2D(_MainTex, i.texcoord) * _DiffuseFactor;
